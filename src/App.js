  ...
  useEffect(() => {
    const initAuth = async () => {
      if (!isDemoEnv && manualConfig.apiKey.includes("請填入")) {
        showMsg("error", "尚未設定 Firebase！請修改程式碼中的 manualConfig");
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        showMsg("error", `登入失敗: ${err.message}`);
      }
    };
    initAuth();
    ...
    const unsubscribe = onSnapshot(
      ...
      (err) => {
        ...
        if (err.code === "permission-denied") {
          showMsg("error", "權限不足：請到 Firebase Console 開啟 Test Mode");
        } else if (err.code === "resource-exhausted") {
          showMsg("error", "配額不足或檔案過大");
        } else {
          showMsg("error", "資料同步失敗");
        }
        setLoading(false);
      }
    );
...
function NavButton({ active, onClick, icon, label }) {
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center gap-1 p-2 rounded-xl transition-all w-1/4 ${
        active
...
// --- 入庫與出庫共用表單 ---
function TransactionForm({ mode, inventory, onSave }) {
  const [formPartNumber, setFormPartNumber] = useState(""); // 改用料號搜尋
...
// --- 庫存查詢頁面 (合併功能版) ---
function InventorySearch({ inventory, onSave, isDemoEnv }) {
  ...
  const handleBatchImageUpload = (e) => {
    ...
    if (
      !confirm(`確定要匯入 ${files.length} 張圖片嗎？將依據「料號」自動配對。`)
    ) {
      e.target.value = null;
      return;
    }
    ...
    const checkDone = () => {
      ...
      if (processedCount === files.length) {
        onSave(
          "success",
          `圖片匯入完成：成功配對 ${successCount} 張 (料號)，${failCount} 張無對應料號`
        );
        e.target.value = null;
      }
    };
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editingItem) {
        ...
        await setDoc(
          doc(db, "artifacts", appId, "public", "data", "inventory", newId),
          data
        );
    ...
    <div className="animate-in fade-in h-full flex flex-col">
        ...
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2">
            {currentFolder && !globalSearch ? (
              <button
                ...
                className="text-indigo-600 font-bold flex items-center gap-1 hover:bg-indigo-50 px-2 py-1 rounded-lg transition-colors"
              >
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
              ...
              <>
                ...
                {(currentFolder || globalSearch) && (
                  <button
                    onClick={
                      isBatchEditMode
                        ? () => setShowConfirmBatchSave(true)
                        : toggleBatchEditMode
                    }
                    className={`p-1.5 px-3 rounded-lg text-xs font-bold flex items-center gap-1 shadow-sm active:scale-95 transition-colors ${
                      isBatchEditMode
            ...
            <button
              onClick={() =>
                exportToCSV(
                  displayItems.length > 0 ? displayItems : inventory,
                  `庫存清單`
                )
              }
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 lg:grid-cols-8 gap-4">
          {folders.map((f) => (
            ...
            >
              <FolderOpen
                size={32}
                className="text-blue-400 fill-blue-50 group-hover:text-blue-500"
              />
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden animate-in slide-in-from-right-4 relative">
          ...
          {isDeleteMode && (
            <div className="absolute top-0 left-0 right-0 bg-red-50 p-2 flex justify-between items-center z-10 border-b border-red-200">
              ...
              <div className="flex gap-2">
                <button
                  onClick={toggleDeleteMode}
                  className="bg-white text-slate-600 px-3 py-1 rounded-lg text-xs font-bold shadow-sm"
                >
                  取消
                </button>
                <button
                  ...
                  className="bg-red-600 text-white px-3 py-1 rounded-lg text-xs font-bold shadow-sm disabled:opacity-50"
                >
                  確認刪除
                </button>
              </div>
          ...
          {isBatchEditMode && (
            <div className="absolute top-0 left-0 right-0 bg-blue-50 p-2 flex justify-between items-center z-10 border-b border-blue-200">
              <span className="text-blue-700 font-bold text-sm ml-2">
                批次修改模式
              </span>
              <div className="flex gap-2">
                <button
          ...
          <div
            className={`border-b border-slate-200 p-3 flex justify-between items-center ${
            ...
            <table className="w-full text-left text-xs sm:text-sm">
              <thead className="bg-slate-50 text-slate-500 font-semibold border-b border-slate-200 sticky top-0 z-10 shadow-sm">
                <tr>
                  ...
                  <th className="p-2 sm:p-3 whitespace-nowrap w-10 text-center bg-slate-50">
                    序號
                  </th>
                  <th className="p-2 sm:p-3 whitespace-nowrap w-14 bg-slate-50">
                    圖
                  </th>
                  <th className="p-2 sm:p-3 whitespace-nowrap bg-slate-50">
                    料號
                  </th>
                  <th className="p-2 sm:p-3 whitespace-nowrap bg-slate-50">
                    品名
                  ...
                  <th className="p-2 sm:p-3 whitespace-nowrap text-right bg-slate-50">
                    庫存
                  </th>
                  {isEditMode && !isDeleteMode && !isBatchEditMode && (
                    <th className="p-2 sm:p-3 whitespace-nowrap text-center bg-slate-50">
                      操作
                    </th>
                  )}
                </tr>
              ...
              <tbody className="divide-y divide-slate-100">
                {displayItems.map((item, index) => {
                  ...
                  const editData = isBatchEditMode
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-2 sm:p-3 text-center text-slate-400 font-mono text-xs">
                        {index + 1}
                      </td>
                        ...
                        <div className="w-10 h-10 bg-white border border-slate-200 rounded-md overflow-hidden flex items-center justify-center shadow-sm">
                          {item.photo ? (
                            <img
                              ...
                              className="w-full h-full object-contain"
                              onClick={() =>
                                !isBatchEditMode && setPreviewImage(item.photo)
                              }
                            />
                          ) : (
                      ...
                      {isBatchEditMode ? (
                        ...
                        // 一般檢視模式
                        <>
                          <td className="p-2 sm:p-3 font-bold text-slate-700">
                            {item.partNumber}
                          </td>
                          <td className="p-2 sm:p-3 font-bold text-slate-700">
                            {item.name}
                          ...
                          <td className="p-2 sm:p-3 text-slate-600 text-xs">
                            {item.remarks || "-"}
                          </td>
                          <td
                            className={`p-2 sm:p-3 text-right font-mono font-bold ${
                ...
                {displayItems.length === 0 && (
                  <tr>
                    <td
                      colSpan={isEditMode ? (isDeleteMode ? 12 : 11) : 10}
      ...
      {isAdding && isEditMode && (
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
          <form
            onSubmit={handleSave}
            className="bg-white w-full max-w-sm p-6 rounded-3xl shadow-2xl space-y-4 animate-in zoom-in-95 max-h-[90vh] overflow-y-auto"
          >
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <select
                    ...
                    className="w-24 p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none"
                  >
            ...
            <button
              type="submit"
              className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold shadow-lg shadow-indigo-200 transition-transform active:scale-95 mt-4"
            >
              確認儲存
            </button>
          </form>
