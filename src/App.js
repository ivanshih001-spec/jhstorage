...
export default function App() {
  ...
  useEffect(() => {
    ...
    // B. 監聽分類 (恢復功能，這裡保留全域監聽以便首頁使用，各資料夾專屬監聽在 InventorySearch 內)
...
function InventorySearch({ inventory, onSave, isDemoEnv, currentUser }) {
  ...
  const [categories, setCategories] = useState(['零件', '成品']); // 本地分類狀態

  // 監聽並同步目前資料夾的分類設定 (區域性)
  useEffect(() => {
    // 預設分類
    const defaultCats = ['零件', '成品'];
  ...
  // 修正變數名稱：將 formData 拆解為獨立 state，確保與 openAddModal 中的設定一致
  const [formPartNumber, setFormPartNumber] = useState('');
