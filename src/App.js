    ...
    <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95">
        <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-indigo-50">
           <h3 className="font-bold text-indigo-900 flex items-center gap-2"><Settings size={20}/> 分類管理</h3>
...
export default function App() {
  ...
  const [categories, setCategories] = useState(['零件', '成品']); // 恢復分類狀態
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-24 relative">
      ...
      <main className="max-w-7xl mx-auto p-4 w-full">
        ...
        {activeTab === 'search' && <InventorySearch inventory={inventory} categories={categories} onSave={showMsg} isDemoEnv={isDemoEnv} currentUser={user} />}
      </main>
  ...
  useEffect(() => {
    if (matchingVariants.length > 0) {
      setSelectedAttr(prev => ({
        ...
        category: categories.length > 0 ? categories[0] : '', // 預設使用第一個分類
...
// --- 庫存查詢頁面 (合併功能版) ---
function InventorySearch({ inventory, categories, onSave, isDemoEnv, currentUser }) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const data = {
        ...
        quantity: parseInt(formQty)||0, safetyStock: parseInt(formSafetyStock)||5000, photo: formPhoto,
        lastUpdated: new Date().toISOString(), lastEditor: currentUser.email
      };
    ...
    <div className="animate-in fade-in h-full flex flex-col">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            ...
            {isEditMode && <>
               ...
               {/* 分類管理按鈕 */}
               <button onClick={()=>setShowCatManager(true)} className="bg-gray-100 text-gray-600 p-1.5 px-2 rounded-lg text-xs font-bold hover:bg-gray-200"><Settings size={14}/></button>
      ...
      {isAdding && isEditMode && (
        <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
          <form onSubmit={handleSave} className="bg-white w-full max-w-sm p-6 rounded-3xl shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
             ...
             <div className="relative"><input type="file" accept="image/*" className="hidden" id="modal-photo" onChange={handleModalPhoto} /><label htmlFor="modal-photo" className="block w-full p-2 border border-dashed rounded text-center text-slate-500 cursor-pointer"><Camera size={20} className="inline mr-1"/>{formPhoto?'更換':'上傳'}照片</label>{formPhoto && <img src={formPhoto} className="mt-2 w-full h-32 object-contain bg-slate-100 rounded"/>}</div>
