...
function ImagePreviewModal({ images, onClose }) {
  ...
  // 處理 images 可能是單張(string)或多張(array)
    ...
    if (sortConfig.key) {
       list = [...list].sort((a, b) => {
         ...
         if (key === 'size') {
            ...
            if (res === 0) res = (a.spec || '').localeCompare(b.spec || '', "zh-Hant");
         } else {
             ...
             <table className="w-full text-left text-xs sm:text-sm">
               <thead className="bg-slate-50 font-semibold border-b sticky top-0 z-10 shadow-sm">
                 <tr>
                   ...
                   {['序號','圖','料號','品名'].map(h=><SortHeader key={h} label={h} sortKey={h==='料號'?'partNumber':h==='品名'?'name':null} />)}
